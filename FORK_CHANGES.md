...
- Windows: execve нет — откат на launcher_restart_required (ручной рестарт, как раньше).
- Фронт: диалог честно предупреждает «лаунчер перезапускается сам, попросит войти» (логины морды в памяти) и сам перезагружает страницу через 8 секунд — на экран логина. i18n ru/en/zh.

## Волна 76 — История чатов v3 из bot_memory.db (D-AUDIT-109) · 19 авг 2026

- Морда читала историю из файлового стора (~/.picoclaw/workspace/sessions), который v3 не пишет вообще — история была пуста всегда, у всех. Переведена на bot_memory.db: список чатов из messages (заголовок = кастомный из registry chat_title: или первые 60 символов первого сообщения, счётчик, даты), деталь чата, поиск ?q= через messages_fts.
- PikaSessionStore реализует MetadataAwareSessionStore: EnsureSessionMetadata пишет маппинг chat_scope:<sk_v1_> → pico:<uuid> в registry (kind=snapshot, без новых таблиц). Существующий вызов в agent loop ожил; раз-skipped тест TestAgentLoop_AgentForSession_UsesStoredScopeMetadata разблокирован и зелёный.
- Удаление из истории = только скрытие (registry chat_hidden:), сообщения в базе не трогаются никогда (правило founder'а, покрыто тестом). Одиночное и групповое (чекбоксы) скрытие в UI.
- Переименование: PATCH /api/sessions/{id} + карандаш в списке. Старые чаты без маппинга — read-only просмотр с баннером (чтобы не форкнуть пустой чат). Найдено тестом: FTS5 матчит целые слова без RU-стемминга («погода» ≠ «погоду») — префиксный поиск в бэклог.

## Волна 75 — Поиск по сообщениям на FTS5+BM25 (D-AUDIT-106) · 19 авг 2026

- LIKE+счётчик слов+свежесть топили факты: бой 19 авг — «синий» утонул в ~13 свежих «не знаю» с теми же словами. Переведено на индустриальный стандарт: messages_fts (FTS5 external content) + bm25() — IDF: слово из 1 документа весит больше слова из 13. Паттерн = knowledge_fts/events_archive_fts.
- Миграция v3: таблица + триггеры (insert/delete/update) + rebuild-бэкфилл существующих. Scope-фильтр chat_id сохранён (JOIN на messages). Слой messages теперь FTS (IsFTS=true) и участвует в общей BM25-нормализации scoreResults. Удалённые атомайзером ходы исчезают из поиска триггером.

## Волна 74 — Атомайзер: порог по умолчанию 100K (D-AUDIT-106) · 19 авг 2026

- 800K не достигался никогда (бой: knowledge_atoms=0). Дефолт → 100K по решению founder'а: DefaultAtomizerConfig + fallback в NewAtomizer + тест.
- Догон бэклога безопасен: порог — сумма токенов per chat; Run берёт старейшие ходы чанками ≤200K за прогон и архивирует → дренируется постепенно, гигантского прогона нет.
...
- Бой: /memory all сработала (флаг записан), но цвет не нашёлся. Два корня, оба из боя:
  (1) AND по всем словам запроса давал ноль на развёрнутых LLM-запросах («любимый цвет пользователя Gar» ≠ «мой любимый цвет — синий») → searchMessages перешёл на OR по словам + ранг по числу совпавших, перелимит ×4 и отрезка.
  (2) /memory писала флаг под сырым msg.ChatID («pico:<uuid>»), а сообщения/поиск живут под canonical sk_v1_<sha256> → флаг не читался, scope молча оставался session. Команда теперь берёт opts.Dispatch.SessionKey (fallback msg.ChatID).
- Сверка боем: факт «синий» лежит в messages (chat sk_v1_5176…, 14 строк с «цвет»); после фикса флаг пишется как memory_scope:sk_v1_… — совпадает с chat_id.
- Тесты: scope default/set, изоляция, кросс-чат all, полное совпадение, OR-частичное, развёрнутый LLM-запрос. Пакет pkg/pika зелёный.
- Попутно в бэклог: registry засорён дублями correction_rule от Рефлектора (нет дедупа) — отдельная задача.

## Волна 71 — Фикс кнопки самообновления: лишний шаг фронта (D-AUDIT-105 follow-up) · 18 авг 2026

- Бой: первый клик по «Обновить из main» → 500. Причина: make build-frontend есть только в web/Makefile, эндпоинт звал её из корня → no rule. Шаг удалён: make build-launcher сам гоняет vite-сборку фронта.
- Плюс волна 70.5 (отдельный PR): slash-палитра переведена из absolute в поток — контейнер композера с overflow её срезал (в DOM была, невидима).
...
- Переключатель НА ЧАТ: /memory [all|session] в чате; флаг в registry (kind=snapshot, key=memory_scope:<chat_id>) — новых таблиц нет (CHECK registry пускает 4 kind; вариант с таблицей отклонён founder'ом). Дефолт session (конкретика не покидает чат, пока чат не включил all).
- Правило слоёв (founder): атомы/инструменты — глобальны; сырая конкретика messages — по scope. Уважают оба читателя: search_memory и search_context Архивариуса.
- Плюс: многословный AND в messages; WARN-лог вместо молчания при ошибке поиска сообщений у Архивариуса.
- Тесты: scope default/set/invalid, изоляция чатов, кросс-чат в all, многословный AND (hit/miss). Пакет pkg/pika зелёный.
- Примечание-долг: атомы при trigger_tokens=800K не рождаются в обычных чатах (knowledge_atoms=0 в бою) — отдельная задача.

## Волна 67 — Русская локаль веб-консоли (D-AUDIT-100) · 18 авг 2026

- Новый web/frontend/src/i18n/locales/ru.json: меню, вход/настройка пароля, чат, модели, ключи, каналы (Telegram и др.), наши страницы (Субагенты, Агенты). Редкие глубокие описания покрыты fallback'ом на en (i18next fallbackLng) — добиваем по факту использования.
- i18n/index.ts: зарегистрирована локаль ru + dayjs locale ru. Автоопределение языка браузера уже было (LanguageDetector) — русскоязычный браузер переключается сам.
- Гейты: vite build (регенерация routeTree) + pnpm build с typecheck.
...
- Полный прогон gosec (401 находка на 112k строк) разобран вручную: G704 SSRF (utils/http_retry, utils/download) — ложные, универсальные HTTP-помощники, SSRF-гейт живёт в web_fetch; G703 path traversal — ложные (пути из конфига; skills.go защищён ValidateSkillIdentifier — режет /, \, ..); G115/G404/G101 — upstream-шум вне нашей зоны (каналы qq/weixin/serial/asr, джиттер ретраев). pkg/pika: точечный прогон гейта нашёл 27 — все разобраны и закрыты этим же PR: G202 x6 (ложные: конкатенируются только «?»-плейсхолдеры из счётчика, значения параметризованы — #nosec с обоснованием), G304 x5 (ложные: пути из конфига, D-90 — #nosec), G104 x14 (механика _ = Close()), G301+G306 (реальные мелочи: права exports 0755/0644 -> 0750/0600, пофикшено).
...
## Волна 88 — Архивариус: обязательный поиск + честный контракт пустого брифа · 20 авг 2026

- **Бой 20 авг (телеметрия):** `tool_calls_requested=0` на всех вызовах Архивариуса — модель НЕ вызывала search_context вообще, сразу выдавала пустой JSON (42 токена). Промт учил: «пустой бриф — нормально, основная модель справится»; код карал: пустой BriefText = hard error → весь MEMORY BRIEF выкидывался.
- **archivist_build.md:** Шаг 1 помечен ОБЯЗАТЕЛЬНЫМ (минимум один search_context до JSON); пример пустой памяти уточнён (пустота — только ПОСЛЕ реального вызова); пункт 0 самопроверки; recency-напоминание исправлено.
- **context_pika.go:** пустой бриф = отсутствие вклада (nil, nil), а не ошибка — контракт кода согласован с промтом.
- **archivist.go:** телеметрия фан-аута — INFO-лог с числом хитов по аспектам на каждый search_context (видно в /logs).

## Волна 89 — Чип сборки на морде (anti-stale) · 20 авг 2026

- **Бой 20 авг (корень потери полдня):** `make build-launcher` собирает только морду; ядро (`build/picoclaw`) собирает `make build`. Диалоги обслуживал старый бинарь, хотя main был зелёным — ничто в UI это не показывало.
- **Backend /api/system/version:** + launcher_commit/launcher_build_time (ldflags лаунчера), gateway_binary_path + gateway_binary_mtime, флаг stale (коммит ядра ≠ коммиту лаунчера; dev/пустые не триггерят). Декорация после кэша — всегда свежая.
- **Frontend:** чип в AppHeader — хеш коммита ядра; красный + тултип «ядро старее лаунчера → make build» при stale. Обновляется при смене состояния gateway.
- **Тест:** TestIsStaleBuild (6 кейсов).
- **Известная граница:** коммит читается с бинаря на диске, не с процесса — «пересобрал, но не рестартнул» чип не ловит (отдельный хвост, нужен /health ядра).

## Волна 90 — Сигнал в шуме: дедуп ДО отсечки + no-JSON retry · 20 авг 2026

- **Бой 20 авг (проба bm25 на боевой БД):** 12 вариантов вопроса-эха со скором -8.62 заняли весь топ; факт-ответ не влез. Корень глубже пунктауции: SQL LIMIT отсекал ДО дедупа в Go — факт не выезжал из базы в принципе.
- **memory_tools.go:** over-fetch ×4 (SQL тянет с запасом) + tie-break по m.id (ничьи bm25 недетерминированы) + normalizeContentKey (регистр/пунктуация/пробелы не различают — индустриальный паттерн нормализация+хеш).
...
## Волна 95 — Кнопка «Обновить из main» = полный рестарт · 21 авг 2026

- **Бой 21 авг 01:05:** после обновления кнопкой чип ложно краснел: ядро на диске обновилось (мерж 94), gateway рестартнул, но лаунчер-процесс остался старым — relaunch срабатывал только при изменениях web/ (D-AUDIT-110), а волна 94 трогала только pkg/pika. Детектор stale (ядро-на-диске ≠ лаунчер-в-процессе) честно закричал — но зря.
- **source_update.go:** relaunch лаунчера на POSIX теперь ВСЕГДА после успешного обновления (механика та же — exec поверх себя, тот же PID/терминал, attach к gateway по PID-файлу). LauncherRestartRequired = !relaunching (Windows — ручной рестарт после любого обновления). В лог кнопки добавлен список изменённых файлов и флаг web changed.
- Зеркальный случай боя 20 авг закрыт: тогда «собрал морду, забыл ядро», теперь «обновилось ядро — перезапустится и морда».

## Волна 96 — selfRelaunch заговаривает · 21 авг 2026
...
## Волна 121 (срез C) — гонка старта: OAuth-рефреш синхронно до подъёма гейтвея · 25 сен 2026

- **Корень (бой 25 сен 18:27)**: первый прогон тикера StartIntegrationRefresher идёт асинхронно (main.go:592, горутина), гейтвей стартует через 1s (main.go:657): initialize MCP с протухшим OAuth-токеном → Unauthorized, сервер мёртв до рестарта. Реактивный 401-путь среза А покрывает вызовы ПОДКЛЮЧЁННОГО сервера, но не провалившийся initialize.
- **web/backend/api/gateway.go** — MODIFIED: TryAutoStartGateway вызывает h.refreshIntegrationsSync() СИНХРОННО до PID-проверки и старта. При запасе > integrationRefreshBuffer — no-op (конфиг с диска, без сети); худший случай (оба провайдера протухли + сеть висит) — до ~60s задержки старта (30s таймаут × 2).
