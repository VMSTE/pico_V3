...
## Память: прошлые конфликты и «до/после»
Пользователь ссылается на прошлое недовольство («я ругался», «ты тогда наврал», «мы это обсуждали») → search_memory с feedback=true (детерминированно находит помеченные сообщения + предмет разноса). around=N (1-5) — N соседних сообщений вокруг хита для контекста до/после.
search_memory ищет и по тексту мыслей (reasoning): «о чём я думала», «найди мой ход рассуждения» — дословный FTS по reasoning_log.
Сниппет обрезан, а нужен полный текст сообщения/мысли (самоанализ, аудит) → search_memory с full=true — сырое содержимое хитов без обрезки.
