...
// === PIKA-V3 Phase 2 tests ===

func TestResolveAgentConfig_FullPikaMerge(t *testing.T) {
	temp := 1.0
	zeroTemp := 0.0
	trueVal := true
	cfg := &Config{
		Agents: AgentsConfig{
			...
			List: []AgentConfig{
				{
					ID: "atomizer", Name: "\u0410\u0442\u043e\u043c\u0438\u0437\u0430\u0442\u043e\u0440",
					Model:          &AgentModelConfig{Primary: "background"},
	...
	r := cfg.ResolveAgentConfig("atomizer")

	if r.Temperature != 0.0 {
		t.Errorf("Temperature = %v, want 0.0", r.Temperature)
	}
	if r.ModelName != "background" {
		t.Errorf("ModelName = %q, want background", r.ModelName)
	}
	if r.MemoryDBPath != "/workspace/memory/bot_memory.db" {
		t.Errorf("MemoryDBPath = %q, want /workspace/memory/bot_memory.db", r.MemoryDBPath)
	}
	if r.TriggerTokens != 800000 {
		t.Errorf("TriggerTokens = %d, want 800000", r.TriggerTokens)
	}
	if r.IdleTimeoutMin != 30 {
		t.Errorf("IdleTimeoutMin = %d, want 30 (inherited)", r.IdleTimeoutMin)
	}
	if !r.Enabled {
...
func TestDefaultConfig_PikaDefaults(t *testing.T) {
	cfg := DefaultConfig()
	if cfg.Agents.Defaults.ContextManager != "pika" {
		t.Errorf("ContextManager = %q, want pika", cfg.Agents.Defaults.ContextManager)
	}
	if cfg.Agents.Defaults.MemoryDBPath == "" {
		t.Error("MemoryDBPath should not be empty")
	}
	if !cfg.Clarify.Enabled {
		t.Error("Clarify should be enabled by default")
	}
	if !cfg.Security.RAD.Enabled {
		t.Error("RAD should be enabled by default")
	}
	if cfg.Health.WindowSize != 5 {
		t.Errorf("Health.WindowSize = %d, want 5", cfg.Health.WindowSize)
	}
	if !cfg.Tools.BaseTools.Enabled {
		t.Error("BaseTools should be enabled by default")
	}
}

func TestConfig_IsBaseToolEnabled(t *testing.T) {
	cfg := DefaultConfig()
