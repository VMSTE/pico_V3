...
`,
	...
	registry := NewAgentRegistry(cfg, &mockRegistryProvider{}, nil)
...
func TestContextBuilder_BuildMessagesOmitsAgentDiscoveryWithoutSpawnTool(t *testing.T) {
	t.Skip("PIKA-PORT: requires per-agent tool allowlist from AGENT.md frontmatter" +
		" (upstream v0.2.9) — deliberately not ported, see D-AUDIT-96")
	mainWorkspace := setupWorkspace(t, map[string]string{
		"AGENT.md": `---
description: Main agent
tools: [read_file]
